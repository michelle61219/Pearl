...
export default {
    // Recieves the tempProduct from ProductsView.vue
    props: {
        // tempProduct
        product: {
            // Expected Object
            type: Object,
            // Failed to recieve will return empty {}
            default() {
                return {};
            }
        },
    },
    //監聽每次傳送的物件是否有不同，不同可更新資料
    watch: {
        product() {
            // 監聽空物件裝傳進來的資料
    ...
    methods: {
        uploadFile() {
            // 綁上DOM fileInput, 取出在files[0]的檔案內容
